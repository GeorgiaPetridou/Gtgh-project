...
//					}
	...
	// Search With Stream
	public List<Event> searchEvents(String theme, String location, Integer day, String month, Integer year) {
		return events.stream().filter(event -> location == null || event.getLocation().equals(location))
				.filter(event -> theme == null || event.getTheme().equals(theme))
				.filter(event -> (day == null && month == null && year == null) || (event.getDay().equals(day)
						&& event.getMonth().equals(month) && event.getYear().equals(year)))
	...
	public List<Event> updateEvent(Integer idEvent, String title, String theme, String description, String location,
			Integer maxCapacity, Integer day, String month, Integer year, Integer hour, Integer minute,
			Organizer organizer, String status, Integer duration) {
		...
		for (Event e : events) {
			...
			// throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Event with id " +
			// idEvent + " doesnt exist");
